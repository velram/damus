...
enum MentionRef: TagKeys, TagConvertible, Equatable, Hashable {
    ...
    var bech32: String {
        switch self {
        case .pubkey(let pubkey): bech32_pubkey(pubkey)
        case .note(let noteId):   bech32_note_id(noteId)
        }
    }
    ...
    var tag: [String] {
        switch self {
        case .pubkey(let pubkey): ["p", pubkey.hex()]
        case .note(let noteId):   ["e", noteId.hex()]
        }
    }
